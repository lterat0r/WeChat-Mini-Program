...
plt.figure(2), plt.imshow(thresh, cmap='gray'),
plt.title('阈值二值化'), plt.axis('off')
plt.figure(3), plt.imshow(sure_bg, cmap='gray'),
plt.title('膨胀'), plt.axis('off')
plt.figure(4), plt.imshow(dist_tran, cmap='gray'),
plt.title('距离变换'), plt.axis('off')
plt.figure(5), plt.imshow(surface_fg, cmap='gray'),
plt.title('种子区域'), plt.axis('off')
plt.figure(6), plt.imshow(unknown, cmap='gray'),
plt.title('边界区域'), plt.axis('off')
plt.figure(7), plt.imshow(np.abs(markers), cmap='rainbow'),
plt.title('标记区域'), plt.axis('off')
plt.figure(8), plt.imshow(cv.cvtColor(img13, cv.COLOR_BGR2RGB)),
